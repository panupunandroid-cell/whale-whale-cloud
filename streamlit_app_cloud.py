...
def get_sheet_id_from_secrets():
    # รองรับทั้งแบบอยู่ด้านนอก และอยู่ใน gcp_service_account
...
def get_workbook():
    ...
    if not sheet_id:
        st.error(
            ...
            "เปิดหน้า *Settings → Secrets* แล้วเพิ่มบรรทัดนี้:\n\n"
    ...
    try:
        ...
        st.error(
            "❌ หา Google Sheets ไม่เจอจาก sheet_id นี้\n"
            "ตรวจสอบว่า sheet_id ตรงกับรหัสหลัง `/d/` ใน URL ของไฟล์"
        )
        st.stop()
    except APIError as e:
        st.error(
            "❌ Service Account ยังไม่มีสิทธิ์เข้าไฟล์นี้\n"
            "ให้แชร์ไฟล์ Google Sheets ให้ email ของ Service Account และตั้งสิทธิ์เป็น Editor\n\n"
            f"รายละเอียด: {e}"
        )
        st.stop()
...
def load_income_df():
    ...
    # ถ้ามีคอลัมน์รวมต่อวันแล้ว ให้คำนวณใหม่ทับ / ถ้าไม่มีให้สร้างใหม่
...
def update_income_row(day, cash, scan, half, grab, shopee, lineman):
    ...
    for i in range(1, len(data)):
        ...
        try:
            d = int(float(v))
            if d == day:
                target_row = i + 1  # เพราะ index เริ่ม 1 = header
                break
        except Exception:
            continue
